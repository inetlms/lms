...
foreach ($short_to_longs as $short => $long)
    if (is_array($options) && array_key_exists($short, $options)) {
...
$quiet = (is_array($options)) ? array_key_exists('quiet', $options) : false;
if (!$quiet) {
    print <<<EOF
lms-sendinvoices.php
(C) 2001-2012 LMS Developers
...
if (is_array($options) && array_key_exists('config-file', $options))
...
require_once(LIB_DIR . '/swiftmailer/lib/swift_required.php');
...
if (!empty($CONFIG['sendinvoices']['smtp_port']))
    $CONFIG['mail']['smtp_port'] = intval($CONFIG['sendinvoices']['smtp_port']);
...
if (!empty($CONFIG['sendinvoices']['smtp_auth']))
    $CONFIG['mail']['smtp_auth_type'] = strtolower($CONFIG['sendinvoices']['smtp_auth']);
...
if (empty($CONFIG['mail']['smtp_auth_type']))
...
if (!empty($docs)) {
    ...
    foreach ($docs as $doc) {
        ...
        if (!empty($res)) {
            ...
            if (!$quiet || $test)
                ...
                // 'protocol' => $CONFIG['mail']['smtp_auth_type'],
            ...
            if (!$test) {
                $transport = Swift_SmtpTransport::newInstance($CONFIG['mail']['smtp_host'], $CONFIG['mail']['smtp_port'], $CONFIG['mail']['smtp_auth_type'])
                    ->setUsername($CONFIG['mail']['smtp_username'])
                    ->setPassword($CONFIG['mail']['smtp_password']);

                $message = Swift_Message::newInstance();
                $message->setTo(array($custemail => $doc['name']));
                $message->setSubject($subject);
                $message->setBody($body, $CONFIG['sendinvoices']['mail_body_type']);
                //$message->addPart($res, $ftype);
                $message->setFrom($from, $CONFIG['sendinvoices']['mail_sent_from']);
                $message->attach(Swift_Attachment::newInstance($res, $filename . '.' . $fext, $ftype));

                $mailer = Swift_Mailer::newInstance($transport);

                if (!$mailer->send($message, $failures)) {
                    fprintf(STDERR, "Error sending mail:" . $failures[0] . "\n");
                } else {
                    $log .= "[" . date("d.m.Y H:i:s") . "] Sent message to $custemail with subject \"$subject\"\n";
                }
            }
        }
    }
    ...
    if (strlen($log) > 1) {
        ...
        $message = Swift_Message::newInstance();
        $message->setTo(array($from => $CONFIG['sendinvoices']['mail_sent_from']));
        $message->setSubject($CONFIG['sendinvoices']['mail_log_subject']);
        $message->setBody('<pre>' . $log . '</pre>', $CONFIG['sendinvoices']['mail_body_type']);;
        $message->setFrom($from, $CONFIG['sendinvoices']['mail_sent_from']);
        $mailer = Swift_Mailer::newInstance($transport);
