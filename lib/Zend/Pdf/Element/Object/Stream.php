...
require_once LIB_DIR.'/Zend/Pdf/Element/Stream.php';
...
class Zend_Pdf_Element_Object_Stream extends Zend_Pdf_Element_Object
{
    ...
    private function _decodeStream()
    {
        ...
        foreach ($this->_initialDictionaryData['Filter'] as $id => $filterName ) {
            ...
            switch ($filterName) {
                case 'ASCIIHexDecode':
                    require_once LIB_DIR.'/Zend/Pdf/Filter/AsciiHex.php';
                    $valueRef = Zend_Pdf_Filter_AsciiHex::decode($valueRef);
                    break;

                case 'ASCII85Decode':
                    require_once LIB_DIR.'/Zend/Pdf/Filter/Ascii85.php';
                    $valueRef = Zend_Pdf_Filter_Ascii85::decode($valueRef);
                    break;

                case 'FlateDecode':
                    require_once LIB_DIR.'/Zend/Pdf/Filter/Compression/Flate.php';
                    $valueRef = Zend_Pdf_Filter_Compression_Flate::decode($valueRef,
                                                                          $this->_initialDictionaryData['DecodeParms'][$id]);
                    break;

                case 'LZWDecode':
                    require_once LIB_DIR.'/Zend/Pdf/Filter/Compression/Lzw.php';
                    $valueRef = Zend_Pdf_Filter_Compression_Lzw::decode($valueRef,
                                                                        $this->_initialDictionaryData['DecodeParms'][$id]);
                    break;

                case 'RunLengthDecode':
                    require_once LIB_DIR.'/Zend/Pdf/Filter/RunLength.php';
