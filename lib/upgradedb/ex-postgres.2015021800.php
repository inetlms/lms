...
if (!$DB->GetOne('SELECT 1 FROM pg_tables WHERE tablename = ? ;',array('netdeviceproducers')))
{
$DB->Execute("
...
if (!$DB->GetOne('SELECT 1 FROM information_schema.columns WHERE table_name = ?  AND column_name=? ;',array('netdevices','netdevicemodelid'))) 
{
...
$DB->Execute("UPDATE dbinfo SET keyvalue = ? WHERE keytype = ?", array('2015021800', 'dbversion'));
