...
if (!$DB->GetOne("SELECT 1 FROM information_schema.columns WHERE table_name = ? AND column_name= ?;",array('customers','ctying')))
    $DB->Execute("ALTER TABLE customers ADD ctying INTEGER DEFAULT 0;"); 	// data rozwiąznia umowy

if (!$DB->GetOne("SELECT 1 FROM information_schema.columns WHERE table_name = ? AND column_name= ?;",array('customers','dtying')))
