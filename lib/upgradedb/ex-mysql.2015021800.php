...
if (!$tmp = $DB->GetOne("SELECT 1 FROM INFORMATION_SCHEMA.TABLES WHERE table_name = ? AND table_schema = ? LIMIT 1",array('netdeviceproducers',$DB->_dbname))) 
{
$DB->Execute("
...
if (!$DB->GetOne("SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS WHERE table_schema = ? AND TABLE_NAME = ? AND COLUMN_NAME = ? ;",array($DB->_dbname,'netdevices','netdevicemodelid'))) 
{
...
$DB->Execute("UPDATE dbinfo SET keyvalue = ? WHERE keytype = ?", array('2015021800', 'dbvex'));
