...
//		if (strtolower($CONFIG['phpui']['report_type']) == 'pdf') 
		if (get_conf('phpui.report_type') == 'pdf') 
		{
			$output = $SMARTY->fetch('module:printcustomertraffic.html');
			html2pdf($output, trans('Reports'), $layout['pagetitle']);
		} else {
			$SMARTY->display('module:printcustomertraffic.html');
		}
