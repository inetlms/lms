...
if($_GET['id_rates'])
{
	...
	$layout['pagetitle'] = 'VoIP Nettelekom - '.$voip->wsdl->GetTrunkgrpName($_GET['id']) . ' - ' . $rate[0]['desc'];
	$hours = $voip->wsdl->GetTrunkHourDetails2($_GET['id'], $_GET['id_rates']);
}
else
{
	$layout['pagetitle'] = 'VoIP Nettelekom - '.$voip->wsdl->GetTrunkgrpName($_GET['id']) . ' - ' . $voip->rategroups[$_GET['c']];
