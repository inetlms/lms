...
if( !($id = $DB->GetOne('SELECT id FROM networknodegroups WHERE id = ?', array(intval($_GET['id'])))))
{
	$SESSION->redirect('?m=networknodegrouplist');
}


$nodegroup = $LMS->GetNetworkNodeGroup($id);
$nodes = $LMS->GetNetworkNodesWithoutGroup($id);
...
if(isset($_POST['nodegroup']))
{
	...
	if(!$error)
	{
		$LMS->DB->Execute('UPDATE networknodegroups SET name=?, description=?
				WHERE id=?',
...
$SMARTY->assign('nodegroups', $LMS->GetnetworkNodeGroupNames());
$SMARTY->display('networknodegroupedit.html');
