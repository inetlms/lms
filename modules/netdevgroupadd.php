...
if(isset($_POST['nodegroupadd']))
{
	...
	elseif($DB->GetOne('SELECT 1 FROM netdevicesgroups WHERE UPPER(name) = ?', array(strtoupper($nodegroupadd['name']))))
		$error['name'] = 'Podana nazwa już istnieje';

	if(!$error)
	{
		$DB->Execute('INSERT INTO netdevicesgroups (name, description)
				VALUES (?, ?)', 
		...
		if (isset($nodegroupadd['reuse'])) 
		{
			...
			$SMARTY->display('netdevgroupadd.html');
		} 
		
		$id = $DB->GetLastInsertID('netdevicesgroups');
...
$layout['pagetitle'] = 'Nowa grupa dla interfejsu sieciowego';
