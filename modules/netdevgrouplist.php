...
function GroupList()
{
	...
	if($nodegrouplist = $DB->GetAll('SELECT id, name, description, 
	    	        (SELECT COUNT(*)
	                FROM netdevicesassignments
	                WHERE netdevicesgroupid = netdevicesgroups.id
			) AS nodescount
	                FROM netdevicesgroups ORDER BY name ASC'))
	{
...
$layout['pagetitle'] = 'Lista grup interfejsów sieciowych';
...
$SMARTY->display('netdevgrouplist.html');
