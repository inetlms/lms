...
if($_GET['id_rates'])
{
	...
	$layout['pagetitle'] = 'VoIP Nettelekom - '.$voip->wsdl->GetCennName($_GET['id']) . ' - ' . $rate[0]['desc'] . ' - edytuj godziny';
}
else
	$layout['pagetitle'] = 'VoIP Nettelekom - '.$voip->wsdl->GetCennName($_GET['id']) . ' - ' . $voip->rategroups[$_GET['c']] . ' - edytuj godziny';
