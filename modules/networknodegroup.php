...
if($action == 'delete')
{
        $DB->Execute('DELETE FROM networknodeassignments WHERE networknodeid=? AND networknodegroupid=?',
                	array(intval($_GET['id']), intval($_GET['nodegroupid'])));
}
elseif($action ==  'add')
{
	if($LMS->NetworkNodeExists(intval($_GET['id']))
		&& $DB->GetOne('SELECT id FROM networknodegroups WHERE id = ?', array($_POST['nodegroupid'])))
                $DB->Execute('INSERT INTO networknodeassignments (networknodeid, networknodegroupid)
            		VALUES (?, ?)', array(intval($_GET['id']), intval($_POST['nodegroupid'])));
}
elseif(!empty($_POST['marks']) && $DB->GetOne('SELECT id FROM networknodegroups WHERE id = ?', array(intval($_GET['groupid']))))
{
	foreach($_POST['marks'] as $mark)
		if($action == 'unsetgroup')
			$DB->Execute('DELETE FROM networknodeassignments
					WHERE networknodegroupid = ? AND networknodeid = ?',
					array(intval($_GET['groupid']), $mark));
		elseif($action == 'setgroup')
		{
			if(!$DB->GetOne('SELECT 1 FROM networknodeassignments
					WHERE networknodegroupid = ? AND networknodeid = ?',
					array(intval($_GET['groupid']), $mark)))
				$DB->Execute('INSERT INTO networknodeassignments 
					(networknodegroupid, networknodeid) VALUES (?, ?)',
					array(intval($_GET['groupid']), $mark));
		}
}
elseif(isset($_POST['nodeassignments']) && $DB->GetOne('SELECT id FROM networknodegroups WHERE id = ?', array($_GET['id'])))
{
	$oper = $_POST['oper'];
